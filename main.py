...
ПРИМЕРЫ:
① Загущение полное, 55 см: 150 кап × 1.6 = 240 + 60г × 7.95 = 477 → ИТОГО ~717 BYN
② Удлинение средние, 60 см: 270 × 1.6 = 432 + 130г: 858 + 257 + 30доп = 1145 → ИТОГО ~1577 BYN
③ Удлинение густые, 65 см: 330 × 1.6 = 528 + 175г: 920 + 690 + 30доп = 1640 → ИТОГО ~2168 BYN
    ...
    async def cb_ai(cb: CallbackQuery, state: FSMContext):
        ...
        if paid_consults.get(uid):
            ...
            await cb.message.edit_text(
                ...
                "Задай любой вопрос по наращиванию волос:\n"
                "• Расчёт стоимости\n"
                "• Выбор метода\n"
                "• Уход за наращенными\n"
                "• Подводные камни\n\n"
                "<i>← Выйти — для возврата в меню</i>",
                reply_markup=ai_kb()
            )
        else:
            await cb.message.edit_text(

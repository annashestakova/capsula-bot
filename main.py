...
async def main():
    ...
    async def cb_free_guide(cb: CallbackQuery):
        ...
        await cb.message.edit_text(
            ...
            "━━━━━━━━━━━━━━━━\n"
            "Это только верхушка. В полном гайде — всё по делу, 10 страниц без воды 👇\\n\\n"
            "💎 <b>Гайд «Секреты долгой носки» — 9 BYN</b>\\n\\n"
            "Что внутри — только то что реально работает:\\n"
            "✦ Мытьё: техника пенки, правильные движения сверху вниз, обязательный кондиционер каждый раз\\n"
            "✦ После мытья: полотенце 15 мин → несмываемый уход → Loop Brush → фен по протоколу\\n"
            "✦ Укладка: TIGI After Party + термозащита — порядок нанесения который меняет всё\\n"
            "✦ Сон и расчёсывание: почему один неправильный рывок = смещённая капсула\\n"
            "✦ Стоп-лист: 8 вещей которые тихо убивают наращивание\\n"
            "✦ Лайфхаки мастера — спутанные пряди, электризация, море, окрашивание\\n"
            "✦ Идеальная неделя ухода по дням: от процедуры до коррекции\\n"
            "✦ График по неделям: когда ждать коррекцию и что делать каждый месяц\\n\\n"
            "🗣 <i>«Носила 4,5 месяца — ни одна капсула не выпала»</i>\\n\\n"
            "👇 9 BYN — окупается уже на первой процедуре.",
        )
        await cb.answer()

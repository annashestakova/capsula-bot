...
async def main():
    ...
    async def start(message: Message, state: FSMContext):
        ...
        await message.answer(
            ...
            "📍 Брест · <a href=\"https://www.instagram.com/volos_capsula/\">@volos_capsula</a>\n\n"
            "✨ Подберу метод · 💰 Рассчитаю · 📅 Запишу\n"
            "🎁 Новым клиентам — купон <b>-30 BYN</b> на первое наращивание\n"
            "Придёт автоматически после записи 💜\n\n"
            "Выбирай 👇",
            reply_markup=main_menu()
        )
    ...
    async def book_confirm(cb: CallbackQuery, state: FSMContext, bot: Bot):
        ...
        # Купон только новым клиентам (первая запись)
        uid = cb.from_user.id
        user_bookings = [k for k,v in bookings.items() if v.get("user_id") == uid]
        is_new_client = len(user_bookings) == 1  # только что созданная — первая
        if is_new_client:
            promo = f"НОВАЯ-{uid}"
            try:
    ...
    async def cb_free_guide(cb: CallbackQuery):
        b = InlineKeyboardBuilder()
        b.row(InlineKeyboardButton(text="💎 Хочу полный гайд — 9 BYN", callback_data="buy_guide"))
        b.row(InlineKeyboardButton(text="📅 Записаться на наращивание", callback_data="book"))
        b.row(InlineKeyboardButton(text="← Меню", callback_data="menu"))
        await cb.message.edit_text(
            "🔒 <b>Почему капсулы сползают — и при чём тут ты, а не мастер</b>\n\n"
            "Я работаю с волосами 5 лет. И знаешь, что говорят почти все когда приходят с жалобой? "
            "\"Наверное, плохо закрепили\". Нет. В 9 случаях из 10 — это уход.\n\n"
            "<b>Вот что происходит на самом деле 👇</b>\n\n"
            "После мытья хочется убрать волосы — и ты тянешь их в резинку мокрыми. "
            "Особенно когда они чуть подросли и начали путаться у корней. "
            "Один резкий рывок — и капсула поехала вниз. "
            "Ты не замечаешь сразу. Замечаешь через неделю, когда уже колтун.\n\n"
            "Или шампунь. Льёшь прямо из флакона на голову — "
            "концентрат стекает точно по капсулам и размягчает крепление. "
            "Каждое мытьё — минус немного стойкости. "
            "Через месяц пряди держатся уже не так.\n\n"
            "Это не страшилки — это то, что я вижу каждую неделю на коррекциях.\n\n"
            "━━━━━━━━━━━━━━━━\n"
            "В гайде я собрала всё: что делать и в каком порядке, "
            "какие средства реально работают, что незаметно убивает наращивание, "
            "и лайфхаки которые я даю только своим клиенткам лично.\n\n"
            "💎 <b>«Секреты долгой носки» — 9 BYN</b>\n"
            "10 страниц практики. Написано как будто я сижу рядом и объясняю 💜\n\n"
            "🗣 <i>«Носила 4,5 месяца — ни одна капсула не выпала»</i>",
            reply_markup=b.as_markup()
        )
        await cb.answer()
    ...
    async def cb_buy_guide(cb: CallbackQuery):
        await cb.message.edit_text(
            "💎 <b>Гайд «Секреты долгой носки наращивания»</b>\n\n"
            "10 страниц. Только практика — без воды:\n\n"
            "✅ Мытьё: техника пенки, движения строго сверху вниз\n"
            "✅ После мытья: полотенце → Loop Brush → протокол фена\n"
            "✅ Средства: HADAT, TIGI After Party, L'Oréal — зачем каждое\n"
            "✅ Укладка, сон и расчёсывание — всё по шагам\n"
            "✅ Стоп-лист: 8 вещей которые убивают наращивание\n"
            "✅ Лайфхаки мастера + идеальная неделя ухода\n\n"
            "💰 Стоимость: <b>9 BYN</b>\n\n"

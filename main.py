...
Пример 1 — Загущение полное, 55 см:
- Капсул: ~150 шт → работа: 150 × 1.6 = 240 BYN
- Волос: ~60г → (60/100) × 795 = 477 BYN
- ИТОГО: ~717 BYN

Пример 2 — Удлинение средние волосы, 60 см:
- Капсул: ~270 шт → работа: 270 × 1.6 = 432 BYN
- Волос: ~130г → (100/100)×858 + (30/100)×858 + 30(доплата) = 858 + 257 + 30 = 1145 BYN
- ИТОГО: ~1577 BYN

Пример 3 — Удлинение густые волосы, 65 см:
- Капсул: ~330 шт → работа: 330 × 1.6 = 528 BYN
- Волос: ~175г → (100/100)×920 + (75/100)×920 + 30(доплата) = 920 + 690 + 30 = 1640 BYN
- ИТОГО: ~2168 BYN

ФОРМАТ ОТВЕТА НА ВОПРОС О ЦЕНЕ:
1. Уточни тип (загущение/удлинение) и густоту волос если не указано
2. Дай диапазон капсул и граммов
3. Посчитай: работа + волосы = итого
4. Всегда пиши "~" и "точный расчёт после осмотра"
5. Предложи записаться через кнопку

ВАЖНО:
- Всегда показывай расчёт пошагово: работа отдельно + волосы отдельно = итого
...
async def main():
    ...
    async def cb_free_guide(cb: CallbackQuery):
        ...
        await cb.message.edit_text(
            ...
            "Натуральный славянский волос 60см стоит от 858 BYN за 100г. "

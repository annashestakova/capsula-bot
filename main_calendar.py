...
AI_SYSTEM = f"""Ты — ассистент мастера по наращиванию волос Анны (@capsula_volos, Брест).
Отвечай тепло, по-русски, 2-4 предложения.
ЦЕНЫ: Наращивание капсульное 1.6 BYN/капсула + волосы(45см=729,50=760,55=790,60=853,65=915,70=961,75=1054,80=1116).
Биопротеиновое 350-400 BYN. Ленточное от 200 BYN. Снятие капсул 0.4/прядь. Коррекция от 80 BYN.
Запись через кнопку в боте."""
...
async def main():
    ...
    async def cb_free_guide(cb: CallbackQuery):
        ...
        await cb.message.edit_text(
            ...
            "<b>1. Больно?</b> Нет — капсулы крепятся без боли.\n\n"
            "<b>2. Красить?</b> Только корни. Нарощенные не красить.\n\n"
            "<b>3. Сколько держится?</b> Капсульное 3-4 мес, ленточное 2-3 мес.\n\n"
            "<b>4. Бассейн, сауна?</b> Бассейн в шапочке. Сауна ограниченно.\n\n"
            "<b>5. Пропустить коррекцию?</b> Капсулы сместятся, волосы начнут путаться.\n\n"
            "💜 Хочешь полный гайд по уходу? 9 BYN 👇",
            reply_markup=b.as_markup()
        )
        await cb.answer()
